...
class CsaSolver(Solver):
    ...
    def run(self, devices_to_schedule, seed=None, max_iter=100):  # max_iter is passed here
    ...
    def run_csa(self, devices,  # This 'devices' parameter is the list of device names (e.g., ["Dishwasher [kW]", ...])
                n_crows=150, P_aw=0.3, seed=None, FL=1, max_iter=100):  # max_iter is correctly received here
        # Removed the redundant hardcoded assignment `max_iter=100` that was overriding the passed argument.
        ...
        for _ in range(max_iter):  # Loop now uses the received max_iter
