...
def csa_reel(planning_initial, n_corbeaux=10, iterations=100):
    corbeaux = [planning_initial.copy() for _ in range(n_corbeaux)]
    memoire = corbeaux.copy()
    fitness_memoire = [fitness(c) for c in corbeaux]

    meilleure_solution_globale = corbeaux[fitness_memoire.index(min(fitness_memoire))]
    meilleure_fitness_globale = min(fitness_memoire)
        ...
        for i in range(n_corbeaux):
            ...
            for appareil in corbeaux[i]:
                if random.random() < 0.5:
                    # Suivre la meilleure solution de corbeau j
                    nouveau_corbeau[appareil] = memoire[j][appareil].copy()
                else:
                    # Perturbation aléatoire
                    nouveau_corbeau[appareil] = corbeaux[i][appareil].copy()
                    nouveau_corbeau[appareil]['heure_debut'] = random.randint(0, 23)

            nouvelle_fitness = fitness(nouveau_corbeau)

            if nouvelle_fitness < fitness_memoire[i]:
                memoire[i] = nouveau_corbeau
                fitness_memoire[i] = nouvelle_fitness

                if nouvelle_fitness < meilleure_fitness_globale:
                    meilleure_solution_globale = nouveau_corbeau
                    meilleure_fitness_globale = nouvelle_fitness

    return meilleure_solution_globale, meilleure_fitness_globale
...
planning_optimise, fitness_optimise = csa_reel(planning_initial)
